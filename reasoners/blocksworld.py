...
import reasoners.bw_utils as bw_utils
...
class BWEvaluator(Evaluator):
    ...
    def sample_prompt(self, shuffle_prompt=True, num_shot=4):
        ...
        if sample_prompt_type == "rap":
            if shuffle_prompt:
                examples = random.sample(self.init_prompt["example_pool"],
                                         num_shot)
            else:
    ...
    def eval_output(self, answer, output):
        bw_utils.text_to_plan_blocksworld(output, answer["instance_file"],
                                          self.config_file, self.domain_file,
                                          self.lm_plan_file)

        correct = bw_utils.validate_plan(self.domain_file,
